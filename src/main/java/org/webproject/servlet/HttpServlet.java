...
/* C:\Users\Stacey Marion\IdeaProjects\Lab_5\src\main\java\org\webproject\servlet\HttpServlet.java */
...
public class HttpServlet extends javax.servlet.http.HttpServlet {
    ...
    // Won't use doGet in our example
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    }

    // We will use doPost for protections
    // Sets JSON and HTTP response format
    protected void doPost(HttpServletRequest request, HttpServletResponse
            response) throws ServletException, IOException {
        response.setContentType("application/json");
        response.setCharacterEncoding("UTF-8");

        String tab_id = request.getParameter("tab_id");  // parameter is key string
        ...
        // query reports
            ...
            try {
                ...
                e.printStackTrace();   // TODO autogenerated catch block
            } catch (SQLException e) {
                e.printStackTrace(); // TODO autogenerated catch block
            }
        }
    ...
    private void createReport(HttpServletRequest request, HttpServletResponse   // server handler method
            response) throws SQLException, IOException {
        DBUtility dbutil = new DBUtility();  // creates a DBUtility object
        String sql;

        // 1. create emergency contact
        int contact_id = 0;   // use contact_id to store id of the contact in our person table; id is autogenerated
        String contact_fN = request.getParameter("contact_fN");   // get data specified in ajax data field of POST request
        String contact_lN = request.getParameter("contact_lN"); // note: storing as strings
        String contact_tel = request.getParameter("contact_tel");
        String contact_email = request.getParameter("contact_email");
        if (contact_fN != null) {contact_fN = "'" + contact_fN + "'";}  // null gets single quote, per SQL notation
        ...
        if (contact_fN != null && contact_lN != null) {
            // create the contact if user has contact; insert into person table
            sql = "insert into person (first_name, last_name, telephone, email) " +
                    "values (" + contact_fN + "," + contact_lN + "," + contact_tel + ","
                    + contact_email + ")";
            dbutil.modifyDB(sql);   // use modifyDB method

            // record the contact id by querying db. will be used for user creation
        ...
        // 2. create user in the person table
        int user_id = 0;
        ...
        // record user_id; will be used for report creation
        ...
        sql = "insert into report (reportor_id, report_type, disaster_type, geom," +
                ...
                message + ")";  // don't use quotes for lon and lat because numbers
        dbutil.modifyDB(sql);

        // record report_id
        ResultSet res_3 = dbutil.queryDB("select last_value from report_id_seq");
        res_3.next();
        report_id = res_3.getInt(1);  // store report_id; to be used for donation/request/damage report creation

        System.out.println("Success! Report created.");

        // 4. create specific report.  sql query specific to type
    ...
    private void queryReport(HttpServletRequest request, HttpServletResponse   // second server handler fxn
            response) throws JSONException, SQLException, IOException {
        JSONArray list = new JSONArray();   // use array to represent response, i.e. list of reports

        String disaster_type = request.getParameter("disaster_type"); // specify three query parameters
        String report_type = request.getParameter("report_type");
        // resource_or_damage will be null if report_type is null
        String resource_or_damage = request.getParameter("resource_or_damage");

        // query each one by one ; add result to JSONArray list
        // request report, if report_type null, queries all
        if (report_type == null || report_type.equalsIgnoreCase("request")) {
            String sql = "select report.id, report_type, resource_type, " +
                    ...
                    "report_id";
            queryReportHelper(sql,list,"request",disaster_type,resource_or_damage); // avoid code duplication
        }
